...
public class AutoWallsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cmdSender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0){
            cmdSender.sendMessage(plugin.wprefix + ChatColor.AQUA + "AutoWalls v." + ChatColor.YELLOW + plugin.version);
        ...
        if (args[0].equalsIgnoreCase("reload")) {
            if (cmdSender.isOp() || cmdSender.hasPermission("walls.op")){
                plugin.reloadConfig();
                plugin.saveConfig();
                cmdSender.sendMessage(plugin.wprefix + ChatColor.GREEN + "Config reloaded!");
                return  true;
            }
        }
