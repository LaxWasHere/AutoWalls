...
public class TellCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cmdSender, Command cmd, String label, String[] args) {
        ...
        if (cmdSender instanceof Player)
        {
            ...
            if (args.length < 2) { cmdSender.sendMessage(plugin.wprefix + ChatColor.RED + "Usage : /tell [name] [message]"); return true; }
            String msg="";
            boolean first = true;
            for (String s : args)
            {
        ...
        {
            if (args.length < 2) { cmdSender.sendMessage(plugin.wprefix + ChatColor.GRAY + "Usage : /tell [name] [message]"); return true; }
            String msg="";
            boolean first = true;
            for (String s : args)
            {
