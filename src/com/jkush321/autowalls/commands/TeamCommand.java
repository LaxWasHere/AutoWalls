    ...
    public boolean onCommand(CommandSender cmdSender, Command cmd, String label, String[] args) {
        ...
        if (cmdSender instanceof Player) {
            ...
            if (!plugin.playing.contains(p)) {
                p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "You are not in game and have no team!");
                return true;
            } else {
                if (plugin.redTeam.contains(p)) {
                    p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "You are on the red team with...");
                    ...
                    p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "You are on the blue team with...");
                    ...
                    p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "You are on the green team with...");
                    ...
                    p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "You are on the orange team with...");
                    for (Player pl : plugin.orangeTeam) {
                        if (pl != p) p.sendMessage(ChatColor.YELLOW + "-" + pl.getName());
                    }
                    if (plugin.orangeTeam.size() == 1) p.sendMessage(plugin.wprefix + ChatColor.YELLOW + "No one else :[");
                }
                return true;
            }
        }
