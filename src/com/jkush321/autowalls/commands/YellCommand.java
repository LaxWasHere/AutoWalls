...
public class YellCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cmdSender, Command cmd, String label, String[] args) {
        ...
        if (plugin.config.getInt("votes.players." + cmdSender.getName()) >= 20 || !(cmdSender instanceof Player) || cmdSender.hasPermission("walls.op"))
        {
            ...
            else cmdSender.sendMessage(plugin.wprefix + ChatColor.AQUA + "Usage : /yell [message]");
        }
        else cmdSender.sendMessage(plugin.wprefix + ChatColor.AQUA + "You need at least 20 priority to do that.");
        return true;
    }
}